browser.on("init", function () {
    ...
    function hexStrToRGBA(hexStr) {
        ...
        hexStr = hexStr.replace("#", "");//replace() 方法用于在字符串中用一些字符替换另一些字符，或替换一个与正则表达式匹配的子串

        if (hexStr.length === 6) {
            // No Alpha
            return Object.assign(colorObject, {
                "r": parseInt(hexStr.slice(0, 2), 16),//slice() 方法可从已有的数组中返回选定的元素
                "g": parseInt(hexStr.slice(2, 4), 16),//parseInt() 函数可解析一个字符串，并返回一个整数,解析的数字的基数为16
