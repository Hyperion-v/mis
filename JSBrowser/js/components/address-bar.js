browser.on("init", function () {
    ...
    this.getFavicon = loc => {
        let host = new URI(loc).host; //获得域名
        ...
        asyncOp.oncomplete = e => {
            ...
            let protocol = loc.split(":")[0];//字符串分割成字符串数组
            ...
            this.setFavicon(faviconFallback.shift());//把数组的第一个元素从其中删除,并返回第一个元素的值
        };
        asyncOp.onerror = e => {
    ...
    // Show or hide the progress ring  进度条
    ...
    this.colorizeAddress = address => {
        ...
        let pathname = a.path;
        if (!address.endsWith('/'))
            pathname = pathname.replace(/\/$/, ''); //返回域名
    ...
    this.urlInput.addEventListener("blur", () => getSelection().removeAllRanges()); //清除选中内容
