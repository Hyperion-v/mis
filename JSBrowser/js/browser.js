(function () {
    ...
    // Browser constructor
     function Browser() {
        ...
        this.appView = Windows.UI.ViewManagement.ApplicationView.getForCurrentView(); //代表活动应用程序视图和相关的状态和行为。
    }

    Browser.prototype = {
        constructor: Browser,
        on (type, listener) { //ES6允许直接写入变量和函数，作为对象的属性和方法。这样的书写更加简洁。
...
2、在new实例化后，其成员被实例化，实例对象方可调用。*/
    ...
    addEventListener("DOMContentLoaded", function () { //DOM解析完以后会触发这个事件
        // Get the UI elements
        Object.assign(this, {
        ...
        this.applyFullscreenMode = state => {
            ...
            if (this.isFullscreen) {
                ...
                // Clear the timeout again to ensure there are no race conditions  清除超时并明确没有竞争状态
        ...
        this.navigateTo("www.baidu.com");
    }.bind(browser));

    // Export `browser`
    window.browser = browser; //怎么改??
})();
